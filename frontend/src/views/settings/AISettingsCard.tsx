...
const AISettingsCard = () => {
  ...
  const [aiSegment, setAiSegment] = useState<'analysis' | 'whisper' | 'translation'>('analysis');
  const { ai, setAI, whisperAi, setWhisperAI, translateAi, setTranslateAI } = useSettingStore(
    useShallow((state) => ({
      ai: state.ai,
      setAI: state.setAI,
      ...
      setTranslateAI: state.setTranslateAI,
    }))
  );
    ...
    >
      <div className="px-2 py-0 flex flex-col gap-4">
        <Segmented
          value={aiSegment}
          block
          onChange={(val) => setAiSegment(val as 'analysis' | 'whisper' | 'translation')}
          options={[
            { label: t('settings.ai.analysisTitle'), value: 'analysis' },
            { label: t('settings.ai.whisperTitle'), value: 'whisper' },
            { label: t('settings.ai.translationTitle'), value: 'translation' },
          ]}
        />
        {aiSegment === 'analysis' && (
          <>
        ...
        {aiSegment === 'translation' && (
          <>
            <div className="grid grid-cols-1 md:grid-cols-12 gap-2 items-center">
              <div className="md:col-span-4">
                <Text strong className="block text-[13px] text-gray-600 dark:text-gray-400 mb-0">
                  {t('settings.ai.translationEnabled')}
                </Text>
              </div>
              <div className="md:col-span-8">
                <Switch
                  checked={translateAi.enabled}
                  onChange={(checked) => setTranslateAI({ ...translateAi, enabled: checked })}
                />
              </div>
            </div>
            ...
            {translateAi.enabled && (
              <>
                <div className="grid grid-cols-1 md:grid-cols-12 gap-2 items-center">
                  ...
                  <div className="md:col-span-8">
                    <Select
                      value={translateAi.provider}
                      onChange={(val) => {
                        let baseUrl = translateAi.baseUrl;
                        let modelName = translateAi.modelName;
                        if (val === 'openai') {
                          baseUrl = 'https://api.openai.com/v1';
                          modelName = 'gpt-3.5-turbo';
                        } else if (val === 'anthropic') {
                        ...
                        setTranslateAI({ ...translateAi, provider: val, baseUrl, modelName });
                      }}
                      style={{ width: '100%' }}
                      options={[
                        { value: 'openai', label: 'OpenAI' },
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-2 items-center">
                  ...
                  <div className="md:col-span-8">
                    <Input
                      value={translateAi.baseUrl}
                      onChange={(e) => setTranslateAI({ ...translateAi, baseUrl: e.target.value })}
                      placeholder="https://api.openai.com/v1"
                      className="dark:bg-gray-800 dark:text-gray-300 dark:border-gray-700"
                    />
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-2 items-center">
                  ...
                  <div className="md:col-span-8">
                    <Input.Password
                      value={translateAi.apiKey}
                      onChange={(e) => setTranslateAI({ ...translateAi, apiKey: e.target.value })}
                      placeholder="sk-..."
                      className="dark:bg-gray-800 dark:text-gray-300 dark:border-gray-700"
                    />
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-2 items-center">
                  ...
                  <div className="md:col-span-8">
                    <Input
                      value={translateAi.modelName}
                      onChange={(e) =>
                        setTranslateAI({ ...translateAi, modelName: e.target.value })
                      }
                      placeholder="gpt-3.5-turbo"
                      className="dark:bg-gray-800 dark:text-gray-300 dark:border-gray-700"
                    />
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-2 items-start">
                  ...
                  <div className="md:col-span-8">
                    <Input.TextArea
                      value={translateAi.prompt}
                      onChange={(e) => setTranslateAI({ ...translateAi, prompt: e.target.value })}
                      placeholder={t('settings.ai.promptPlaceholder')}
                      rows={3}
                      className="dark:bg-gray-800 dark:text-gray-300 dark:border-gray-700"
                    />
